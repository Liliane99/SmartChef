...
export default function RegisterPage() {
  ...
  const handleSubmit = () => {
    if (passwordStrength.score < 4 || !passwordsMatch || !formData.acceptTerms) {
      return;
    }
    console.log('Inscription:', formData);
  };

  const isFormValid = passwordStrength.score >= 4 && passwordsMatch && formData.acceptTerms && formData.email && formData.firstName && formData.lastName;
          ...
          <CardContent className="space-y-6">
            ...
            <div className="text-center pt-4">
              <p className="text-muted-foreground">
                Déjà un compte ?{' '}
                <Button onClick={() => window.location.href = "/login"} variant="link" className="p-0 h-auto text-primary hover:text-primary/80 font-semibold">
                  Se connecter
                </Button>
              </p>
            </div>
